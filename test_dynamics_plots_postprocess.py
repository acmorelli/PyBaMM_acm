...
Fa_raw = arr(solution, "Branch A CCPM Flux")
Fb_raw = arr(solution, "Branch B CCPM Flux")
Fc_raw = arr(solution, "Branch C CCPM Flux")
...
fig1 = make_subplots(
    ...
    subplot_titles=("Branch masses", "CCPM stoichiometry", "Terminal voltage",
                    "Positive electrode OCP", "Transition fluxes J_AB, J_BC"),
)
...
fig1.add_trace(go.Scatter(x=time, y=ocp_pos_avg, mode="lines", name="OCP pos [V]"), row=4, col=1)

j_ab_t = np.mean(J_AB, axis=0) if J_AB.ndim > 1 else J_AB
j_bc_t = np.mean(J_BC, axis=0) if J_BC.ndim > 1 else J_BC
fig1.add_trace(go.Scatter(x=time, y=j_ab_t, mode="lines", name="J_AB"), row=5, col=1)
fig1.add_trace(go.Scatter(x=time, y=j_bc_t, mode="lines", name="J_BC"), row=5, col=1)
...
fig1.update_layout(height=1200, width=1100, hovermode="x unified",
                   ...
                   title="C/30 discharge to 2.0V — time series")
fig1.show()
...
def reshape_3d_edge(flat):
    """Reshape edge-valued flux [n_edges*n_x, n_t] -> [n_cp, n_x, n_t] (averaged to nodes)."""
    flat = np.squeeze(flat)
    n_edges = n_cp + 1
    if flat.ndim == 2 and flat.shape[0] == n_edges * n_x:
        edge_3d = flat.reshape((n_edges, n_x, n_t), order="F")
        # Average adjacent edges to get node-centred values
        return 0.5 * (edge_3d[:-1, :, :] + edge_3d[1:, :, :])
    elif flat.ndim == 2 and flat.shape[0] == n_edges:
        edge_3d = flat[:, np.newaxis, :] * np.ones((1, n_x, 1))
        return 0.5 * (edge_3d[:-1, :, :] + edge_3d[1:, :, :])
    return flat
...
Fa_3d = reshape_3d_edge(Fa_raw)
Fb_3d = reshape_3d_edge(Fb_raw)
Fc_3d = reshape_3d_edge(Fc_raw)
...
var_catalog = {
    ...
    "F_a (Branch A flux)": Fa_3d,
    "F_b (Branch B flux)": Fb_3d,
    "F_c (Branch C flux)": Fc_3d,
...
# ── Figure 3: x-resolved time series (OCP, J_AB, J_BC) ──────────────────
def reshape_x(flat):
