...
for rate in rates:
    ...
    axes[1].plot(d["theta"][mask], d["ocp_shift"][mask],
                 linewidth=1.5, color=c, linestyle="-", label=f"{lbl} discharge")
    if mask.any():
...
axes[1].set_ylim(0.0025, y_max_p2 * 1.05)
...
for rate in rates:
    ...
    axes[2].plot(d["theta"][mask], d["ocp_shift"][mask],
                 linewidth=1.5, color=c, linestyle="--", label=f"{lbl} charge")
    if mask.any():
...
axes[2].set_ylim(y_min_p3 * 1.05, -0.0050)
axes[2].legend(fontsize=7, ncol=1)
axes[2].grid(True, alpha=0.3)

plt.savefig("half_cell_fig7_all_rates.png", dpi=150)
plt.close()
print("\nPlot saved to half_cell_fig7_all_rates.png")
